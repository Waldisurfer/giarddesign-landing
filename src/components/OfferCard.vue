...
<template>
  <a
    :id="id"
    :href="href"
    class="group block scroll-mt-24 rounded-card bg-white p-8 transition-[transform,box-shadow] duration-300 ease-out hover:-translate-y-1.5 hover:shadow-card focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-forest lg:p-10"
  >
    ...
    <h3 class="mt-7 font-display text-[24px] font-medium leading-7 tracking-[-0.05em] lg:text-h3">
      {{ title }}
    </h3>
    <p class="mt-3 text-small text-ink/70">{{ copy }}</p>
    <span class="mt-7 inline-flex items-center gap-2 border-b border-forest pb-1 text-body text-forest">
      {{ linkLabel }}
